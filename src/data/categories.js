import { COLORS } from './constants';

export const CATEGORIES = [
  { 
    name: "Everyday Essentials",
    icon: "🌿",
    desc: "Pure everyday spices for authentic cooking",
    items: [
      "Turmeric Powder",
      "Red Chilli Powder",
      "Coriander Powder",
      "Cumin Powder",
      "Dry Ginger Powder",
      "Haldi Powder"
    ],
    color: COLORS.primaryRed
  },

  { 
    name: "Blended Masalas",
    icon: "🫙",
    desc: "Versatile blends for everyday Indian cooking, including marinades",
    items: [
      "Garam Masala",
      "Royal Garam Masala",
      "Kitchen King Masala",
      "Sabji Masala",
      "Chhole Masala",
      "Chana Masala",
      "Rajma Masala",
      "Paneer Masala",
      "Shahi Paneer Masala",
      "Dal Makhani Masala",
      "Goda Masala",
      "Madras Curry Powder",
      "Egg Curry Masala",
      "Pav Bhaji Masala",
      "Shahi Biryani Masala",
      "Chicken Biryani Masala",
      "Hyderabadi Biryani Masala",
      "Sindhi Biryani Masala",
      "Biryani Pulao Masala",
      "Butter Chicken Masala",
      "Chicken Masala",
      "Karahi Chicken Masala",
      "Fish Curry Masala",
      "Meat Masala",
      "Karahi Gosht Masala",
      "Baingan Bharta Masala",
      "Tandoori Chicken Masala",
      "Paneer Tikka Masala",
      "Chicken 65 Masala",
      "Tandoori BBQ Masala",
      "Pani Puri Masala",
      "Jaljeera Powder",
      "Chaas Masala",
      "Chat Masala",
      "Sambhar Masala",
      "Rasam Powder"
    ],
    color: COLORS.gold
  },

  { 
    name: "Ready to Use Mixes",
    icon: "⚡",
    desc: "Quick and delicious ready-to-cook mixes",
    items: [
      "Madras Sambar Mix Powder",
      "Madras Sambar Powder",
      "Tomato Rice Powder",
      "Curry Leaves Rice Powder",
      "Paruppu Podi Dal Rice Powder",
      "Lemon Rice Mix",
      "Drumstick Leaves Idli Podi",
      "Black Pepper Idli Podi",
      "Curry Leaves Idli Podi"
    ],
    color: COLORS.accentOrange
  },

  { 
    name: "Chutneys",
    icon: "🥣",
    desc: "Authentic chutneys for idli, dosa, and snacks",
    items: [
      "Coconut Red Chutney",
      "Coconut Green Chutney",
      "Vada Pav Chutney",
      "Lasoon Chutney",
      "Imli Chutney",
      "Sandwich Chutney",
      "Curd Chilli",
      "Til Chutney Ellu Podi"
    ],
    color: "#8D6E63"
  }
];